...
cl_ulong4 Dispatcher::Device::createSeed() {
    ...
    // 使用设备索引作为偏移量
    const cl_ulong device_offset = static_cast<cl_ulong>(m_index) * 1000000007ULL;

    cl_ulong4 r;
    r.s[0] = distr(eng1) ^ static_cast<cl_ulong>(nanos & 0xFFFFFFFFFFFFFFFFULL);
    r.s[1] = distr(eng2) ^ static_cast<cl_ulong>((nanos >> 32) & 0xFFFFFFFFFFFFFFFFULL) ^ device_offset;
    r.s[2] = distr(eng3) ^ (device_offset << 13);
    r.s[3] = distr(eng4) ^ static_cast<cl_ulong>(nanos) ^ device_offset;
    return r;
#endif
}
...
Dispatcher::Dispatcher(cl_context & clContext, cl_program & clProgram, const Mode mode, const size_t worksizeMax, const size_t inverseSize, const size_t inverseMultiple, const cl_uchar clScoreQuit)
	...
	m_maxSpeed(0.0), m_speedMeasuring(false), m_resetInterval(std::chrono::milliseconds(0)), m_benchmarkMode(false), m_benchmarkDuration(0), m_outputMode(false), m_foundCount(0) {
	
	m_speedMeasureStart = std::chrono::steady_clock::now();
	m_lastResetTime = m_speedMeasureStart;
	m_benchmarkStart = m_speedMeasureStart;
}
...
void Dispatcher::handleResult(Device & d) {
    for (auto i = PROFANITY_MAX_SCORE; i >= 0; --i) {
        ...
        if (r.found > 0) {
            ...
            if (m_outputMode) {
                // 在输出模式下，添加结果到列表
                addResult(strPrivate, strPublic, i);
            } else {
                // 在普通模式下，直接打印结果
                ...
                std::cout << strVT100ClearLine << "  Time: " << std::setw(5) << seconds 
                         << "s Score: " << std::setw(2) << (int)i << " Private: " << strPrivate 
                         << " " << m_mode.transformName() << ": " << strPublic << std::endl;
            }

            // 重置设备的种子
            d.m_clSeed = d.createSeed();
            d.m_round = 0;
            CLMemory<cl_ulong4>::setKernelArg(d.m_kernelInit, 4, d.m_clSeed);
            
            // 运行初始化内核来设置新的起始状态
            cl_event event;
            enqueueKernelDevice(d, d.m_kernelInit, m_size, &event);
...
void Dispatcher::setBenchmarkMode(bool enabled, int durationSeconds) {
	m_benchmarkMode = enabled;
	m_benchmarkDuration = durationSeconds;
	m_benchmarkStart = std::chrono::steady_clock::now();
}

double Dispatcher::getMaxSpeed() const {
	return m_maxSpeed;
}

void Dispatcher::setResetInterval(std::chrono::milliseconds interval) {
	m_resetInterval = interval;
...
void Dispatcher::printSpeed() {
    ...
    if (m_countPrint > m_vDevices.size()) {
        ...
        if (m_benchmarkMode) {
            if (speedTotal > m_maxSpeed) {
                m_maxSpeed = speedTotal;
            }

            auto now = std::chrono::steady_clock::now();
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - m_benchmarkStart).count();
            
            if (elapsed >= m_benchmarkDuration) {
                m_quit = true;
            }
        } else {
        auto now = std::chrono::steady_clock::now();
            if (m_resetInterval.count() > 0 && now - m_lastResetTime >= m_resetInterval) {
                for (auto & device : m_vDevices) {
                    device->m_clSeed = device->createSeed();
                    device->m_round = 0;
                    CLMemory<cl_ulong4>::setKernelArg(device->m_kernelInit, 4, device->m_clSeed);
                    cl_event event;
                    enqueueKernelDevice(*device, device->m_kernelInit, m_size, &event);
                    clWaitForEvents(1, &event);
                    clReleaseEvent(event);
                }
        ...
        // 如果在输出模式下，只显示找到的地址数量，不显示速度
        if (m_outputMode) {
            // 清除当前行并打印地址计数，然后换行
            std::cout << "\33[2K\rFound addresses: " << m_foundCount << std::endl;
        } else {
            // 在非输出模式下，清除当前行并打印速度信息
            std::cout << "\33[2K\rSpeed: " << formatSpeed(speedTotal) << strGPUs << std::flush;
        }
    }
}
